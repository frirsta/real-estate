import homeOne from "../assets/homes/home_one.jpg";
import homeTwo from "../assets/homes/home_two.jpg";
import homeThree from "../assets/homes/home_three.jpg";
import homeFour from "../assets/homes/home_four.jpg";
import homeFive from "../assets/homes/home_five.jpg";
import agentOne from "../assets/agents/agentOne.jpg";
import agentTwo from "../assets/agents/agentTwo.jpg";
import agentThree from "../assets/agents/agentThree.jpg";
import homeSix from "../assets/homeTwo/homeSix.jpg";
import homeSeven from "../assets/homeTwo/homeSeven.jpg";
import homeEight from "../assets/homeTwo/homeEight.jpg";
import homeNine from "../assets/homeTwo/homeNine.jpg";
import homeTen from "../assets/homeTwo/homeTen.jpg";
import agentFour from "../assets/agents/agentFour.jpg";
import agentFive from "../assets/agents/agentFive.jpg";
import agentSix from "../assets/agents/agentSix.jpg";
import agentSeven from "../assets/agents/agentSeven.jpg";
import agentEight from "../assets/agents/agentEight.jpg";
import agentNine from "../assets/agents/agentSix.jpg";
import ApartmentOne from "../assets/featuredHomes/apartment.jpg";
import ApartmentTwo from "../assets/featuredHomes/apartment_two.jpg";
import ApartmentThree from "../assets/featuredHomes/apartment_three.jpg";
import ApartmentFour from "../assets/featuredHomes/apartment_four.jpg";
import ApartmentFive from "../assets/featuredHomes/apartment_five.jpg";
import ApartmentSix from "../assets/featuredHomes/apartment_six.jpg";
import ApartmentSeven from "../assets/featuredHomes/apartment_seven.jpg";
import ApartmentEight from "../assets/featuredHomes/apartment_eight.jpg";
import ApartmentEleven from "../assets/homeThree/homeEleven.jpg";
import ApartmentTwelve from "../assets/homeThree/homeTwelve.jpg";
import ApartmentThirteen from "../assets/homeThree/homeThirteen.jpg";
import ApartmentFourteen from "../assets/homeThree/homeFourteen.jpg";
import ApartmentFifteen from "../assets/homeFour/homeFifteen.jpg";
import ApartmentSixteen from "../assets/homeFour/homeSixteen.jpg";
import ApartmentSeventeen from "../assets/homeFour/homeSeventeen.jpg";
import ApartmentEighteen from "../assets/homeFour/homeEighteen.jpg";

const HomeList = [
  {
    id: "0",
    source:
      "https://www.freepik.com/free-photo/analog-landscape-city-with-buildings_31261387.htm",
    alt1: "image1",
    src1: ApartmentOne,

    source:
      "https://www.freepik.com/free-photo/empty-flat-interrior-with-elements-decoration_10025717.htm",
    alt2: "image2",
    src2: homeTwo,

    source:
      "https://www.freepik.com/free-photo/interior-modern-kitchen-minimalist-style_25773339.htm",
    alt3: "image3",
    src3: homeThree,

    source:
      "https://www.freepik.com/free-photo/modern-spacious-room-with-large-panoramic-window_13908610.htm",
    alt4: "image4",
    src4: homeFour,

    source:
      "https://www.freepik.com/free-photo/3d-rendering-loft-luxury-living-room-with-bookshelf_26833973.htm",
    alt5: "image5",
    src5: homeFive,

    details:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    price: "12 000 000",
    beds: 3,
    baths: 1,
    area: "78m²",
    address: "1177 Biddle Rd,	Medford",
    state: "Indiana",
    agentOne: agentOne,
    agentTwo: agentTwo,
    agentThree: agentThree,
    agentNameOne: "Osbaldo Emmerich",
    agentNameTwo: "August Davis V",
    agentNameThree: "Ernesto Jakubowski",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "1",
    source:
      "Bild av Bianca: https://www.pexels.com/sv-se/foto/trappa-semester-blommor-hotell-1560065/",
    alt1: "image6",
    src1: ApartmentTwo,

    source:
      "https://www.freepik.com/free-photo/modern-luxury-domestic-room-comfortable-relaxation-generative-ai_41074071.htm",
    alt2: "image7",
    src2: homeSeven,

    source:
      "Bild av Jonathan Borba: https://www.pexels.com/sv-se/foto/arkitektur-lagenhet-modern-vardagsrum-9976126/",
    alt3: "image8",
    src3: homeEight,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852631.htm",
    alt4: "image9",
    src4: homeNine,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852655.htm",
    alt5: "image10",
    src5: homeTen,

    details:
      "sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
    price: "22 000 000",
    beds: 4,
    baths: 2,
    area: "150m²",
    address: "1178 Biddle Rd,	Medford",
    state: "Georgia",
    agentOne: agentFour,
    agentTwo: agentFive,
    agentThree: agentSix,
    agentNameOne: "Alberta Nienow",
    agentNameTwo: "	Bettye Kris",
    agentNameThree: "Marcelina Jacobs",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "2",
    source:
      "Bild av Nothing Ahead: https://www.pexels.com/sv-se/foto/stad-semester-hotell-byggnad-3460599/",
    alt1: "image1",
    src1: ApartmentThree,

    source:
      "https://www.freepik.com/free-photo/window-room-with-surreal-mystical-view_40572581.htm",
    alt2: "image2",
    src2: ApartmentEighteen,

    source:
      "https://www.freepik.com/free-photo/white-textile-classical-style-sofa-vintage-room-flowers-ob-painted-barrels_3337208.htm",
    alt3: "image3",
    src3: ApartmentSeventeen,

    source:
      "https://www.freepik.com/free-photo/modern-living-room-design-with-blue-accents-generated-by-ai_41451649.htm",
    alt4: "image4",
    src4: ApartmentSixteen,

    source:
      "https://www.freepik.com/free-photo/blank-picture-frame-by-pink-velvet-armchair_18138417.htm",
    alt5: "image5",
    src5: ApartmentFifteen,

    details:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    price: "12 000 000",
    beds: 3,
    baths: 1,
    area: "78m²",
    address: "1170 Biddle Rd,	Medford",
    state: "Oregon",
    agentOne: agentSeven,
    agentTwo: agentEight,
    agentThree: agentNine,
    agentNameOne: "Letitia Gutkowski",
    agentNameTwo: "Wilfred Tromp",
    agentNameThree: "Micah Fisher",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "3",
    source:
      "Bild av Nothing Ahead: https://www.pexels.com/sv-se/foto/stad-semester-hotell-byggnad-3460599/",
    alt1: "image6",
    src1: ApartmentFour,

    source:
      "https://www.freepik.com/free-photo/modern-living-room-style_18946976.htm",
    alt2: "image7",
    src2: homeSeven,

    source:
      "Bild av Jonathan Borba: https://www.pexels.com/sv-se/foto/arkitektur-lagenhet-modern-vardagsrum-9976126/",
    alt3: "image8",
    src3: homeEight,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852631.htm",
    alt4: "image9",
    src4: homeNine,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852655.htm",
    alt5: "image10",
    src5: homeTen,

    details:
      "sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
    price: "22 000 000",
    beds: 4,
    baths: 2,
    area: "150m²",
    address: "1171 Biddle Rd,	Medford",
    state: "California",
    agentOne: agentFour,
    agentTwo: agentFive,
    agentThree: agentSix,
    agentNameOne: "Wilhelmine Thiel",
    agentNameTwo: "Zella Carroll",
    agentNameThree: "Johanna Botsford",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "4",
    source:
      "https://www.freepik.com/free-photo/design-house-modern-villa-with-open-plan-living-private-bedroom-wing-large-terrace-with-privacy_43175006.htm",
    alt1: "image1",
    src1: ApartmentFive,

    source:
      "https://www.freepik.com/free-photo/empty-flat-interrior-with-elements-decoration_10025717.htm",
    alt2: "image2",
    src2: homeTwo,

    source:
      "https://www.freepik.com/free-photo/interior-modern-kitchen-minimalist-style_25773339.htm",
    alt3: "image3",
    src3: homeThree,

    source:
      "https://www.freepik.com/free-photo/modern-spacious-room-with-large-panoramic-window_13908610.htm",
    alt4: "image4",
    src4: homeFour,

    source:
      "https://www.freepik.com/free-photo/3d-rendering-loft-luxury-living-room-with-bookshelf_26833973.htm",
    alt5: "image5",
    src5: homeFive,

    details:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    price: "12 000 000",
    beds: 3,
    baths: 1,
    area: "78m²",
    address: "1172 Biddle Rd,	Medford",
    state: "California",
    agentOne: agentOne,
    agentTwo: agentTwo,
    agentThree: agentThree,
    agentNameOne: "Osbaldo Emmerich",
    agentNameTwo: "August Davis V",
    agentNameThree: "Ernesto Jakubowski",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "5",
    source:
      "Bild av Gustavo Galeano Maz: https://www.pexels.com/sv-se/foto/lampor-trad-rum-korridor-13752246/",
    alt1: "image6",
    src1: ApartmentSix,

    source:
      "https://www.freepik.com/free-photo/modern-luxury-domestic-room-comfortable-relaxation-generative-ai_41074071.htm",
    alt2: "image7",
    src2: homeSeven,

    source:
      "https://www.freepik.com/free-photo/modern-living-room-style_18946976.htm",
    alt3: "image8",
    src3: homeSix,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852631.htm",
    alt4: "image9",
    src4: homeNine,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852655.htm",
    alt5: "image10",
    src5: homeTen,

    details:
      "sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
    price: "22 000 000",
    beds: 4,
    baths: 2,
    area: "150m²",
    address: "1173 Biddle Rd,	Medford",
    state: "Texas",
    agentOne: agentFour,
    agentTwo: agentFive,
    agentThree: agentSix,
    agentNameOne: "Alberta Nienow",
    agentNameTwo: "	Bettye Kris",
    agentNameThree: "Marcelina Jacobs",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "6",
    source:
      "Bild av Maria Orlova: https://www.pexels.com/sv-se/foto/natur-romantisk-sommar-hus-4913320/",
    alt1: "image1",
    src1: ApartmentSeven,

    source:
      "https://www.freepik.com/free-photo/3d-room-interior-design-with-plants_19924861.htm",
    alt2: "image2",
    src2: ApartmentEleven,

    source:
      "https://www.freepik.com/free-photo/3d-room-interior-design_19924857.htm",
    alt3: "image3",
    src3: ApartmentTwelve,

    source:
      "https://www.freepik.com/free-photo/3d-room-interior-design-with-blue-motifs_19924829.htm",
    alt4: "image4",
    src4: ApartmentThirteen,

    source:
      "https://www.freepik.com/free-photo/3d-room-interior-design-with_19924832.htm",
    alt5: "image5",
    src5: ApartmentFourteen,

    details:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
    price: "12 000 000",
    beds: 3,
    baths: 1,
    area: "78m²",
    address: "1174 Biddle Rd,	Medford",
    state: "California",
    agentOne: agentSeven,
    agentTwo: agentEight,
    agentThree: agentNine,
    agentNameOne: "Letitia Gutkowski",
    agentNameTwo: "Wilfred Tromp",
    agentNameThree: "Micah Fisher",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
  {
    id: "7",
    source:
      "Bild av Bilal Mansuri: https://www.pexels.com/sv-se/foto/byggnad-hus-arkitektur-hem-13041126/",
    alt1: "image6",
    src1: ApartmentEight,

    source:
      "https://www.freepik.com/free-photo/beautiful-interior-shot-modern-house-with-white-relaxing-walls-furniture-technology_8028428.htm",
    alt2: "image7",
    src2: homeOne,

    source:
      "Bild av Jonathan Borba: https://www.pexels.com/sv-se/foto/arkitektur-lagenhet-modern-vardagsrum-9976126/",
    alt3: "image8",
    src3: homeEight,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852631.htm",
    alt4: "image9",
    src4: homeNine,

    source:
      "https://www.freepik.com/free-photo/stylish-scandinavian-living-room-with-design-mint-sofa-furnitures-mock-up-poster-map-plants-eleg_38852655.htm",
    alt5: "image10",
    src5: homeTen,

    details:
      "sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
    price: "22 000 000",
    beds: 4,
    baths: 2,
    area: "150m²",
    address: "1175 Biddle Rd,	Medford",
    state: "Delaware",
    agentOne: agentFour,
    agentTwo: agentFive,
    agentThree: agentSix,
    agentNameOne: "Wilhelmine Thiel",
    agentNameTwo: "Zella Carroll",
    agentNameThree: "Johanna Botsford",
    shortDetails: "Luxury 1, 2 , & 3 Bedroom Apartments",
    startingPrice: "Prices start from $700,000*",
    name: "Sunset field",
  },
];

export default HomeList;
